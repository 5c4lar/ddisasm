...
void PrintLLVM::push_integer(uintb val,int4 sz,bool sign,
...
/// \param vn is the Varnode holding the value
/// \param op is the PcodeOp using the value
void PrintLLVM::pushBoolConstant(uintb val,const TypeBase *ct,
				 const Varnode *vn,
				 const PcodeOp *op)
{
  if (val != 0)
...
/// while still using the basic PrintLLVM functionality
...
/// \param vn is the Varnode holding the value
/// \param op is the PcodeOp using the value
void PrintLLVM::pushCharConstant(uintb val,const Datatype *ct,const Varnode *vn,const PcodeOp *op)
...
/// \param vn is the Varnode holding the value
/// \param op is the PcodeOp using the value
void PrintLLVM::pushEnumConstant(uintb val,const TypeEnum *ct,
				 const Varnode *vn,
				 const PcodeOp *op)
{
...
bool PrintLLVM::pushPtrCharConstant(uintb val,const TypePointer *ct,const Varnode *vn,const PcodeOp *op)
...
bool PrintLLVM::pushPtrCodeConstant(uintb val,const TypePointer *ct,
				    const Varnode *vn,
				    const PcodeOp *op)
{
...
void PrintLLVM::pushConstant(uintb val,const Datatype *ct,
			    const Varnode *vn,
			    const PcodeOp *op)
{
  Datatype *subtype;
...
bool PrintLLVM::pushEquate(uintb val,int4 sz,const EquateSymbol *sym,const Varnode *vn,const PcodeOp *op)
...
void PrintLLVM::pushPartialSymbol(const Symbol *sym,int4 off,int4 sz,
			       const Varnode *vn,const PcodeOp *op,
			       Datatype *outtype)
{
...
void PrintLLVM::pushMismatchSymbol(const Symbol *sym,int4 off,int4 sz,
				const Varnode *vn,const PcodeOp *op)
{
  if (off==0) {
...
void PrintLLVM::emitPrototypeOutput(const FuncProto *proto,
				 const Funcdata *fd)
{
  PcodeOp *op;
  Varnode *vn;
...
/// construct a statement with the appropriate llvm-language keyword (\b goto, \b break, \b continue)
...
void PrintLLVM::emitGotoStatement(const FlowBlock *bl,const FlowBlock *exp_bl,
			       uint4 type)
